...
<body>
	<%
		...
		Board_Content_DTO data = bbsDAO.Get_Board_Content(board_no,bcode);
		String Referrer = request.getHeader("referer");
		int result = 0;
		try{
			result = data.getBcode();
		...
		if(userID == null){
			script.println("<script>");
			script.println("alert('해당 게시글 수정권한이 없습니다.')");
			script.println("location.href='/jap/login/login.jsp'");
			script.println("</script>");
		}
...
<div id="container">
	<form id="boardWriteForm" action="modify_action.jsp" method="post">
		<input id="board_no" name="board_no" value="<%=board_no%>" type="hidden">
		<input id="bcode" name="bcode" value="<%=bcode%>" type="hidden">
		<input id="move_write_after" name="move_write_after" value="<%=Referrer%>" type="hidden">
		<div class="title">
			<label for="title_input">제목</label><input type="text" name="title_input" id="title_input" value="<%=data.getTitle()%>">
		</div>
		<textarea name="contents" id="contents" rows="10" cols="100" style="width:80%; height:412px; display:none;"><%=data.getContents()%></textarea>
		...
		<span class="gRight">
			...
			<a href="read.jsp?board_no=<%=board_no%>&bcode=<%=bcode%>" class="btn_submitboard btncolor_subbasic"><span>취소</span></a>
		</span>
	</form>
</div>
</body>
