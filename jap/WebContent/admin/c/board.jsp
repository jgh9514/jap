		...
		<div id="container">
			...
			<div id="content">
				...
				<%
					...
					ArrayList<Board_Content_DTO> list = bbsDAO.getPostList(0,Ord_default, Sort_default, Rows_default, Page_default-1);
			
					int list_cnt = list.size();
					int total_list_cnt = bbsDAO.count();
				%>
				<form name="form1" method="post" action="board.jsp">
				...
				<table id="member_search">
					...
					<%
		                ...
		                String[] ord_check = {"bcode", "board_id", "title", "userId", "bdate", "hit_count", "vote_count"};
						String[] sort_list = {"","","","","","",""};
		                int ord_count = 0;
		                int sort_count = 0;
		            ...
		            %>
						<tr>
							...
							<th class="menu" onclick="submitSearch('bcode', '<%=sort_list[0]%>')">번호<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('board_id', '<%=sort_list[1]%>')">게시판번호<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('title', '<%=sort_list[2]%>')">제목<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('userId', '<%=sort_list[3]%>')">작성자<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('bdate', '<%=sort_list[4]%>')">작성일<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('hit_count', '<%=sort_list[5]%>')">조회수<span class="sort"></span></th>
							<th class="menu" onclick="submitSearch('vote_count', '<%=sort_list[6]%>')">추천수<span class="sort"></span></th>
						</tr>
						<script>
					...
					%>
						<tr>
							...
							<td><%=list.get(i).getBcode()%></td>
							<td><%=list.get(i).getBoard_id()%></td>
							<td><%=list.get(i).getTitle()%></td>
							<td><%=list.get(i).getUserId()%></td>
							<td><%=list.get(i).getBdate()%></td>
							<td><%=list.get(i).getHit_count()%></td>
							<td><%=list.get(i).getVote_count()%></td>
						</tr>
					<%
						}
					%>
